...
description: Implement or refine AIUP PetClinic use cases using the repository's DDD, Spring Modulith, BDD, Vaadin, jOOQ, and software capability/activity documentation conventions. Use when Codex is asked to add or change a PetClinic use case, update docs/capabilities, generate or maintain Gherkin scenarios, create Cucumber step definitions, or implement Spring Modulith application services from uc.md use-case specs.
---
...
Treat `docs/capabilities/` as the primary source of truth:

- `docs/capabilities/<capability>/entity_model.md` describes the capability's domain model slice.
- `docs/capabilities/<capability>/activities/<activity>/use-cases/<use-case-id>/uc.md` contains the JIRA-backed use-case specification. The JIRA ticket id is recorded inside this file.
- `docs/capabilities/<capability>/activities/<activity>/use-cases/<use-case-id>/uc.feature` contains executable scenarios.
- `docs/capabilities/<capability>/activities/<activity>/use-cases/<use-case-id>/uc.puml` contains the PlantUML use-case and aggregate-interaction diagram.
...
1. Identify the target Software Capability, Software Activity, and use-case id from `docs/capabilities/`.
...
- A Software Capability is the strategic goal boundary: what this software can fundamentally achieve.
- A Software Activity is the behavioral boundary: how related use cases are organized to produce value.
- A Use Case is the delivery boundary: the smallest independently valuable goal.
- A Scenario is the executable flow: how that goal is achieved in one specific situation.
- Activities may own shared policies, domain services, events, or projections. They do not require an application
  service by themselves; application services remain use-case boundaries.
- Traceability chain: Software Capability -> Software Activity -> Use Case / Gherkin Feature -> Gherkin Scenario ->
...
- Software Capabilities, Software Activities, and use cases changed.
